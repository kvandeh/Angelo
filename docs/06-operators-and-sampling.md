# 06 — Operators and sampling

**Abstract.** angelo's operator set was widened to match mutmut's: **63 mutants against
mutmut's 61** on the same file, up from roughly 60%. A bigger pool needs a way to bound
it, so `--sample N` keeps N mutants and **deletes the rest from the database** — the score
becomes an estimate over a random sample of the whole codebase.

## Operators

Every mutation is still a **token-level byte splice**: find a token, replace its bytes.
No AST rewriting, so nothing can go subtly wrong in code generation.
...
- **A method swap needs a dot.** `x.lower()` is a method; a variable called `lower` is not.
- **A `for` loop's `in` is left alone.** `for x not in y` is a syntax error every time, so
...
**This deletes rows.** It is not "run the first 500 and leave the rest pending" — the
overflow leaves the database entirely, so what remains is a random sample drawn from
every file, and the score is an **estimate** over that sample rather than a complete
...
- A small sample is a noisy estimate. The demo scored 62.2% on all 74 mutants and 70.0% on
  a sample of 20 — same code, same tests, 8 points apart.
